    ...
    gfx_state_t *s, uint32_t vmid)
{
    ...
    res = vkResetDescriptorPool(s->vk->device, s->vk->descriptor_pool, 0);
    if (res != VK_SUCCESS) {
        fprintf(stderr, "%s: vkResetDescriptorPool failed (%d)!", __FUNCTION__, res);
        assert(0);
    }
    ...
    gfx_state_t *s, uint32_t vmid, const uint32_t *packet)
{
    ...
    //uint32_t flag = (CP_INT_CNTL_RING0__CNTX_BUSY_INT_ENABLE_MASK | CP_INT_CNTL_RING0__CNTX_EMPTY_INT_ENABLE_MASK);
    //if ((s->mmio[mmCP_INT_CNTL_RING0] & flag) == flag)
