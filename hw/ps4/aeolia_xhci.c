...
#define AEOLIA_XHCI_NODE(obj) \
    OBJECT_CHECK(XHCIState, (obj), TYPE_AEOLIA_XHCI_NODE)
...
static void xhci_process_commands(XHCIState *xhci)
{
    ...
    while ((type = xhci_ring_fetch(xhci, &xhci->cmd_ring, &trb, &addr))) {
        ...
        case CR_VENDOR_NEC_CHALLENGE_RESPONSE:
            ...
            break;
        default:
            fprintf(stderr, "xhci: unimplemented command: %d", type);
            event.ccode = CC_TRB_ERROR;
            break;
        }
        event.slotid = slotid;
...
{
    XHCIState *xhci = AEOLIA_XHCI_NODE(dev);
    int i;
...
static const TypeInfo aeolia_xhci_node_info = {
    .name          = TYPE_AEOLIA_XHCI_NODE,
...
{
    type_register_static(&aeolia_xhci_node_info);
